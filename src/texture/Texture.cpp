...
Texture::Texture (float ***data, unsigned int width, unsigned int height)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, width, height, 0, GL_RGBA, GL_FLOAT, contiguousData);
...
void Texture::updateTexture (float*** data, unsigned int width, unsigned int height)
{
    ...
    // Update the texture
    glBindTexture(GL_TEXTURE_2D, id);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, GL_RGBA, GL_FLOAT, contiguousData);
